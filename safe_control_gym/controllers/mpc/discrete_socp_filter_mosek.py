...
class DiscreteSOCPFilterMOSEK:
    ...
    # @profile
    def compute_feedback_input_mosek(self, z, z_ref, v_des, eta= np.zeros((2,)),  x_init=np.zeros((7,)), **kwargs): 
...
def state_con_matrices(z, gam1, gam2, gam3, gam4, L_gam5, Linv_gam5,
    ...
    b[2, 0] = w_s1*np.sqrt(max((0.5*gam3[0] - (term_Linv_gam4_0[0])**2), 1e-10)) 
    b[3, 0] = w_s1*np.sqrt(max((0.5*gam3[0] - (term_Linv_gam4_0[1])**2), 1e-10)) 
    b[6, 0] = w_s2*np.sqrt(max((0.5*gam3[1] - (term_Linv_gam4_1[0])**2), 1e-10)) 
    b[7, 0] = w_s2*np.sqrt(max((0.5*gam3[1] - (term_Linv_gam4_1[1])**2), 1e-10))
