...
@pytest.mark.parametrize('ALGO', ['ilqr', 'gp_mpc', 'gpmpc_acados', 'gpmpc_acados_TP', 'linear_mpc', 'mpc_acados', 'ppo', 'sac', 'dppo'])
@pytest.mark.parametrize('PRIOR', [''])
@pytest.mark.parametrize('SAFETY_FILTER', ['', 'linear_mpsc'])
@pytest.mark.parametrize('SAMPLER', ['optuna', 'vizier'])
def test_hpo_quadrotor(SYS, TASK, ALGO, PRIOR, SAFETY_FILTER, SAMPLER):
    ...
    if ALGO == 'ilqr' or ALGO == 'gpmpc_acados' or ALGO == 'linear_mpc' or ALGO == 'mpc_acados' or ALGO == 'gpmpc_acados_TP':
        PRIOR = '100'
    elif ALGO == 'gp_mpc':
        PRIOR = '200'
