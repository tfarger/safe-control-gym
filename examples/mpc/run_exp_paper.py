...
yaml_file_fmpc = './config_overrides/quadrotor_2D_attitude/fmpc_ext_quadrotor_2D_attitude_tracking.yaml'
yaml_file_fmpc_socp = './config_overrides/quadrotor_2D_attitude/fmpc_socp_quadrotor_2D_attitude_tracking.yaml'
...
print('\nMaximum of inputs')
print('               NMPC   |  FMPC   | FMPC+SOCP')
print('Thrust: {:.2f}N   | {:.2f}N   | {:.2f}N'.format(np.max(action_mpc[:, 0]), np.max(action_fmpc[:, 0]), np.max(action_fmpc_socp[:, 0])))
