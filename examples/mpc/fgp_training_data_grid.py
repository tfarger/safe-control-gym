...
# # test the flat transformation on the evaluation dataset from the figure 8 with extended dynamics 
...
# v_from_transform = np.zeros(np.shape(v_data))

# for i in range(np.shape(z_data)[1]):
#     v_from_transform[:, i] = _get_v_from_Z_and_u_2D_att_ext(z_data[:, i], u_data[:, i], dyn_pars=inertial_prop, g=g)

# fig, ax = plt.subplots(2)
# ax[0].plot(v_data[0,:], label='reference')
# ax[0].plot(v_from_transform[0,:], label='from transform')
# ax[0].set_title('Flat input v 0 comparison')
# ax[0].legend()

# ax[1].plot(v_data[0,:]-v_from_transform[0,:], label='reference')
# ax[1].set_title('Flat input v 0 differenc1')
# ax[1].legend()

# fig, ax = plt.subplots(2)
# ax[0].plot(v_data[1,:], label='reference')
# ax[0].plot(v_from_transform[1,:], label='from transform')
# ax[0].set_title('Flat input v 1 comparison')
# ax[0].legend()

# ax[1].plot(v_data[1,:]-v_from_transform[1,:], label='reference')
# ax[1].set_title('Flat input v 1 differenc1')
# ax[1].legend()
# plt.show()
